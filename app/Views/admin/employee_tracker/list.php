                                ...
                                <tbody>
                                    ...
                                    if (is_array($emp_details)) {
                                        ...
                                        foreach ($emp_details as $data) {
                                            echo "<tr>";
                                            echo "<td>" . $x . "</td>";
                                            echo'<td><a href="/' . ADMIN_PATH . '/employee/view/' . $data->id . '" class="text-decoration-none">'. ucfirst($data->first_name).' '.ucfirst($data->last_name).'</a></td>';
                                            // echo "<td>" . ucfirst($data->first_name).' '.ucfirst($data->last_name) . "</td>";

                                            foreach ($dates as $dt) {
                                                $dateCategoryData = $dateCategoryDataMap[$data->id] ?? [];
                                                
                                                $btn_clr = '';  // Define $btn_clr here before the inner loop
                                                ...
                                                foreach ($att_category_details as $category) {
                                                    ...
                                                    if (isset($dateCategoryData[$dt]) && $dateCategoryData[$dt] == $category->category_code) {
                                                        $btn_clr = '';
                                                        if ($category->category_code == 'P') {
                                                            // Add the 'text-white bg-success' class
                                                            $btn_clr = 'bg-successs';
                                                        }else if($category->category_code == 'A'){
                                                            $btn_clr = 'bg-dangers';
                                                        }else if($category->category_code == 'CL'){
                                                            $btn_clr = 'bg-lavender';
                                                        }else if($category->category_code == 'CO'){
                                                            $btn_clr = 'bg-orange';
                                                        }else if($category->category_code == 'HD'){
                                                            $btn_clr = 'bg-yellow';
                                                        }else if($category->category_code == 'ML/PL'){
                                                            $btn_clr = 'bg-pink';
                                                        }else if($category->category_code == 'PH'){
                                                            $btn_clr = 'bg-blue';
                                                        }else if($category->category_code == 'WO'){
                                                            $btn_clr = 'bg-grey';
                                                        }else if($category->category_code == 'WFH'){
                                                            $btn_clr = 'bg-purple';
                                                        }else if($category->category_code == 'WH'){
                                                            $btn_clr = 'bg-green';
                                                        }
                                                    }
                                                }
                                                echo '<td class="'.$btn_clr .'"><select class="form-select select-input SelectChange' . $data->id . '" name="category_details" >';
                                                foreach ($att_category_details as $category) {
                                                    // print_r($dateCategoryData[$dt]) ;
