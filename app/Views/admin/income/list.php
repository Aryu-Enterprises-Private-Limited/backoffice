    ...
    <div class="card create-box">
        <div class="card-body">
            ...
            <div class="row">
                ...
                <div class="col-md-1">
                    <a href="<?= (base_url(ADMIN_PATH . '/income/add')); ?>">
...
<script>
    $(function() {
        ...
        if (qryString != '') {
            var url = "<?php echo base_url(); ?>admin/income/list_ajax?" + qryString;
        } else if (qryString2 != '') {
            var url = "<?php echo base_url(); ?>admin/income/list_ajax?" + qryString2;
        } else if (qryString3 != '') {
            var url = "<?php echo base_url(); ?>admin/income/list_ajax?" + qryString3;
        } else {
            var url = "<?php echo base_url(); ?>admin/income/list_ajax";
        }
        ...
        $('#FilterBtns1').on('click', function() {
            var qryString = get_filter_datestrings();
            if (qryString != '') {
                window.location.href = "<?php echo base_url(); ?>admin/income/list?" + qryString;
            } else {
                window.location.href = "<?php echo base_url(); ?>admin/income/list";
            }
        });
        ...
        $('#FilterBtns2').on('click', function() {
            var qryString2 = get_filter_monthstrings();
            if (qryString2 != '') {
                window.location.href = "<?php echo base_url(); ?>admin/income/list?" + qryString2;
            } else {
                window.location.href = "<?php echo base_url(); ?>admin/income/list";
            }
        });
        ...
        $('#FilterBtns3').on('click', function() {
            var qryString3 = get_filter_yearstrings();
            if (qryString3 != '') {
                window.location.href = "<?php echo base_url(); ?>admin/income/list?" + qryString3;
            } else {
                window.location.href = "<?php echo base_url(); ?>admin/income/list";
            }
        });
        ...
        $("#export_all_user").click(function(event) {
            ...
            window.location.href = "<?php echo base_url(); ?>admin/income/list?" + query_strings + "&filter=1&export=excel";
        });
        $('body').on("click", ".btn-close", function() {
            var id = $(this).attr('data-row_id');
            var url = "<?php echo base_url(); ?>admin/income/alert_close";
            $.ajax({
                type: 'post',
                url: url,
                data: {
