...
class Department extends BaseController
{
    ...
    public function index()
    {
        if ($this->checkSession('A') != '') {
            ...
            echo view(ADMIN_PATH . '/department/list', $this->data);
        } else {
    ...
    public function list_ajax($returnType = 'json')
    {
        ...
        foreach ($ajaxDataArr->getResult() as $row) {
            ...
            $actionTxt = '<a class="btn btn-icon text-info" href="/' . ADMIN_PATH . '/department/view/' . (string)$rowId . '"><i class="fas fa-eye"></i></a>';

            $statusTxt =  '<a data-toggle="tooltip" data-original-title="' . $actTitle . '" class="stsconfirm" href="javascript:void(0);" data-row_id="' . $rowId . '" data-act_url="/' . ADMIN_PATH . '/department/change-status" data-stsmode="' . $mode . '"> <button type="button" class="btn ' . $btnColr . ' btn-sm waves-effect waves-light">' . $disp_status . '</button></a>';

            $actionTxt .= '<a class="btn btn-icon " href="/' . ADMIN_PATH . '/department/edit/' . (string)$rowId . '"><i class="fas fa-edit"></i></a>';


            $actionTxt .= '<a href="javascript:void(0);" class="delconfirm btn btn-icon text-danger" data-row_id="' . $rowId . '" data-act_url="/' . ADMIN_PATH . '/department/delete"><i class="fas fa-trash-alt"></i></a>';
        ...
        if ($this->checkSession('A') != '') {
            ...
            if ($id != '') {
                ...
                if (!empty($this->data['dept_info'])) {
                    ...
                    $this->session->setFlashdata('error_message', 'Couldnot find the Department');
                    return redirect()->route(ADMIN_PATH . '/department/list');
                }
            } else {
                $this->data['title'] = 'Add Department';
            }
            echo view(ADMIN_PATH . '/department/add_edit', $this->data);
        } else {
            $this->session->setFlashdata('error_message', 'Please login!!!');
            return redirect()->to('/' . ADMIN_PATH);
        }
    ...
    public function insertUpdate()
    {
        if ($this->checkSession('A') != '') {
            ...
            if ($department_name != '') {
                ...
                if ($id == '') {
                    $this->LmsModel->simple_insert(DEPARTMENT_DETAILS, $dataArr);
                    $this->session->setFlashdata('success_message', 'Department details added successfully.');
                    $fSubmit = TRUE;
                } else {
                    $condition = array('id' => $id);
                    $this->LmsModel->update_details(DEPARTMENT_DETAILS, $dataArr, $condition);
                    $this->session->setFlashdata('success_message', 'Department details update successfully');
                    $fSubmit = TRUE;
                }
            } else {
            ...
            if ($fSubmit) {
                $url = ADMIN_PATH . '/department/list';
            } else {
                if ($id == '') $url = ADMIN_PATH . '/department/add';
                else $url = ADMIN_PATH . '/department/edit/' . $id;
            }
    ...
    public function update_status()
    {
        if ($this->checkSession('A') != '') {
            ...
            if ($this->checkSession('A') == '') {
                ...
                $condition = array('id' => $id);
                $this->LmsModel->update_details(DEPARTMENT_DETAILS, $newdata, $condition);
                $returnArr['status'] = '1';
                $returnArr['response'] = 'Department Status Changed Successfully';
            }
            echo json_encode($returnArr);
            exit;
        } else {
    ...
    public function view($id = "")
    {
        if ($this->checkSession('A') != '') {
            ...
            if ($id != '') {
                ...
                $this->data['deptDetails'] = $this->LmsModel->get_all_details(DEPARTMENT_DETAILS, $condition)->getRow();
                if (!empty($this->data['deptDetails'])) {
                    $this->data['title'] = 'Department view';
                    echo view(ADMIN_PATH . '/department/view', $this->data);
                } else {
                    $this->session->setFlashdata('error_message', 'Couldnot find the Department');
    ...
    public function delete()
    {
        if ($this->checkSession('A') != '') {
            ...
            if ($this->checkSession('A') == '') {
                ...
                if (isset($record_id)) {
                    $this->LmsModel->isDelete(DEPARTMENT_DETAILS, 'id', TRUE);
