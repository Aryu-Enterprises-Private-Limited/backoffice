...
class Report extends BaseController
{
    ...
    public function list_ajax($returnType = 'json')
    {
        ...
        foreach ($groupby->getResult() as $data) {
            $empl_name = '<a href="/' . ADMIN_PATH . '/employee/view/' . $data->employee_id . '" class="text-decoration-none">'. ucfirst($data->employee_name).'</a>';
            $currentDate = date('Y-m-d');
            $currentDate2 = date('Y-m-01');
            $currentMonth = date('n');
            if (isset($daterange) && !empty($daterange)) {
                if (!empty($daterange)) {
                    // $condition['att_current_date'] = $daterange;
                    $dateTime = DateTime::createFromFormat('m/d/Y', $daterange);
                    $currentDate = $dateTime->format('Y-m-d');
                    // $currentDate = $dateTime->format('d');
                    // $currentDate = $dateTime->format('Y-m-01');
            ...
            $cond = ['employee_id' => $data->employee_id, 'MONTH(date)' => $currentMonth];
//   print_r($cond);die;
            ...
            $dayCount = date('d', strtotime($currentDate)) - 1;
            // echo $dayCount;die;
            ...
            if (isset($login) && isset($logout)) {
                ...
                $tblData[] = array(
                    ...
                    'schedule_hrs' => $dailyWorkingHrs[$dayCount],
                    'id' => $login_out_diff,
                    'reason' => $break_diff,
    ...
    public function monthly_list_ajax()
    {
        ...
        if ($this->checkSession('A') != '') {
            ...
            $this->data['emp_details'] = $this->LmsModel->get_selected_fields(EMPLOYEE_DETAILS, $cond, ['id', 'email','first_name'])->getResult();
