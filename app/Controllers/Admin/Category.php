...
class Category extends BaseController
{
    ...
    public function index()
    {
        if ($this->checkSession('A') != '') {
            ...
            echo view(ADMIN_PATH . '/category/list', $this->data);
        } else {
    ...
    public function list_ajax($returnType = 'json')
    {
        ...
        $totCounts = $this->LmsModel->get_all_counts(CATEGORY, $condition, '', $likeArr);
        $sortArr = array('category_name' => -1);
        if ($sortField != '') {
            $sortArr = array($sortField => $sortJob);
        }

        $ajaxDataArr = $this->LmsModel->get_all_details(CATEGORY, $condition, $sortArr, $rowperpage, $row_start, $likeArr);
        ...
        foreach ($ajaxDataArr->getResult() as $row) {
            ...
            $actionTxt = '<a class="btn btn-icon text-info" href="/' . ADMIN_PATH . '/category/view/' . (string)$rowId . '"><i class="fas fa-eye"></i></a>';

            $statusTxt =  '<a data-toggle="tooltip" data-original-title="' . $actTitle . '" class="stsconfirm" href="javascript:void(0);" data-row_id="' . $rowId . '" data-act_url="/' . ADMIN_PATH . '/category/change-status" data-stsmode="' . $mode . '"> <button type="button" class="btn ' . $btnColr . ' btn-sm waves-effect waves-light">' . $disp_status . '</button></a>';

            $actionTxt .= '<a class="btn btn-icon " href="/' . ADMIN_PATH . '/category/edit/' . (string)$rowId . '"><i class="fas fa-edit"></i></a>';


            $actionTxt .= '<a href="javascript:void(0);" class="delconfirm btn btn-icon text-danger" data-row_id="' . $rowId . '" data-act_url="/' . ADMIN_PATH . '/category/delete"><i class="fas fa-trash-alt"></i></a>';
    ...
    public function add_edit($id = "")
    {
        if ($this->checkSession('A') != '') {
            ...
            if ($id != '') {
                ...
                $this->data['cat_info'] = $this->LmsModel->get_selected_fields(CATEGORY, $condition)->getRow();
            ...
            echo view(ADMIN_PATH . '/category/add_edit', $this->data);
        } else {
            $this->session->setFlashdata('error_message', 'Please login!!!');
            return redirect()->to('/' . ADMIN_PATH);
        }
    ...
    public function insertUpdate()
    {
        if ($this->checkSession('A') != '') {
            ...
            if ($category_name != '') {
                ...
                if ($id == '') {
                    $this->LmsModel->simple_insert(CATEGORY, $dataArr);
                    $this->session->setFlashdata('success_message', 'Category added successfully.');
                    $fSubmit = TRUE;
                } else {
                    $condition = array('id' => $id);
                    $this->LmsModel->update_details(CATEGORY, $dataArr, $condition);
                    $this->session->setFlashdata('success_message', 'Category update successfully');
                    $fSubmit = TRUE;
                }
            } else {
            ...
            if ($fSubmit) {
                $url = ADMIN_PATH . '/category/list';
            } else {
                if ($id == '') $url = ADMIN_PATH . '/category/add';
                else $url = ADMIN_PATH . '/category/edit/' . $id;
            }
    ...
    public function update_status()
    {
        if ($this->checkSession('A') != '') {
            ...
            if ($this->checkSession('A') == '') {
                ...
                $condition = array('id' => $id);
                $this->LmsModel->update_details(CATEGORY, $newdata, $condition);
                $returnArr['status'] = '1';
                $returnArr['response'] = 'Category Changed Successfully';
            }
            echo json_encode($returnArr);
            exit;
        } else {
    ...
    public function view($id = "")
    {
        if ($this->checkSession('A') != '') {
            ...
            if ($id != '') {
                ...
                $this->data['category_details'] = $this->LmsModel->get_all_details(CATEGORY, $condition)->getRow();
                if (!empty($this->data['category_details'])) {
                    $this->data['title'] = 'Category view';
                    echo view(ADMIN_PATH . '/category/view', $this->data);
                } else {
    ...
    public function delete()
    {
        if ($this->checkSession('A') != '') {
            ...
            if ($this->checkSession('A') == '') {
                ...
                if (isset($record_id)) {
                    $this->LmsModel->isDelete(CATEGORY, 'id', TRUE);
                    // $this->setFlashMessage('success', 'Lms deleted successfully');
                    $returnArr['status'] = '1';
                    $returnArr['response'] = 'Category Deleted Successfully';
                }
