...
class Income extends BaseController
{
        ...
        if ($this->checkSession('A') != '') {
            ...
            if ($this->request->getPostGet('export') == 'excel') {
                ...
                if ($ajaxDataArr['status'] == '1') {
                    ...
                    $sheet->setCellValue('D1', 'Currency Code');
                    $sheet->setCellValue('E1', 'Amount');
                    $sheet->setCellValue('F1', 'Billed Account');
                    ...
                    foreach ($income->getResult() as $st) {
                        ...
                        $sheet->setCellValue('D' . $rows, strtoupper($st->currency_code));
                        $sheet->setCellValue('E' . $rows, $st->amount);
                        $sheet->setCellValue('F' . $rows, $st->billed_acc_name);
    ...
    public function insertUpdate()
    {
        if ($this->checkSession('A') != '') {
            ...
            $currency_code = (string)$this->request->getPostGet('currency_code');
            ...
            $fSubmit = FALSE;
            if ($date != ''  && $company_name != '' && $billed_acc_name != '' && $amount != '' && $currency_code !='') {
                if ($status == 'on') {
                    $status = '1';
                } else {
                    $status = '0';
