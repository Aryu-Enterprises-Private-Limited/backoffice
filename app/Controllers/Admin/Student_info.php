...
class Student_info extends BaseController
{
    ...
    public function list_ajax($returnType = 'json')
    {
        ...
        foreach ($ajaxDataArr->getResult() as $row) {
            ...
            if(isset($row->follow_up_dt) && !empty($row->follow_up_dt)){
                $followTxt = '<a class="btn btn-icon text-primary" href="/' . ADMIN_PATH . '/student_info/followup_add_edit/' . (string)$rowId . '"><i class="fas fa-edit"></i>Edit</a>';
            }else{
                $followTxt = '<a class="btn btn-icon text-primary" href="/' . ADMIN_PATH . '/student_info/followup_add_edit/' . (string)$rowId . '"><i class="fa fa-plus"></i>Add</a>';
            }
    ...
    public function add_edit($id = "")
    {
        if ($this->checkSession('A') != '') {
            ...
            if ($id != '') {
                ...
                if (isset($url_type) && $url_type == 'followup_add_edit') {
                    $this->data['title'] = 'Add Followup';
                    echo view(ADMIN_PATH . '/student_info/followup_add_edit', $this->data);
                    exit();
                } else if (!empty($this->data['stu_info']) && $url_type != 'followup_add_edit') {
        ...
        public function followup_insertUpdate(){
            if ($this->checkSession('A') != '') {
                // echo"<pre>";print_r($_POST);die;
                $id = (string)$this->request->getPostGet('id');
                $date = $this->request->getPostGet('date');
                $comments = $this->request->getPostGet('comments');

                if (isset($date) && isset($date)) {
                    $revisied_dt_fil = array_values(array_filter($date));
                    $comment_fil = array_values(array_filter($comments));
                    $dataArr['follow_up_dt'] = json_encode($revisied_dt_fil);
                    $dataArr['follow_up_cmnt'] = json_encode($comment_fil);
                } else {
                    $dataArr['revisied_dt'] = '';
                    $dataArr['comment'] = '';
                }
                ...
                $this->session->setFlashdata('success_message', 'Followup update successfully');
